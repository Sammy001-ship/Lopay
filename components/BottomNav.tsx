...
export const BottomNav: React.FC = () => {
  ...
  if (userRole === 'owner') {
    return (
        <div className="fixed bottom-0 left-0 right-0 z-20 mx-auto max-w-md border-t border-gray-200 dark:border-gray-800 bg-white/90 dark:bg-card-dark/90 backdrop-blur-md">
  ...
  if (userRole === 'school_owner') {
    return (
        ...
        <div className="flex h-20 items-center justify-around px-2">
            ...
            className={`flex flex-col items-center gap-1 ${isActive('/school-owner-dashboard') ? 'text-secondary' : 'text-text-secondary-light dark:text-text-secondary-dark'}`}
            >
            <span className={`material-symbols-outlined ${isActive('/school-owner-dashboard') ? 'filled' : ''}`}>bar_chart</span>
            <span className="text-[10px] font-bold">Dashboard</span>
            </button>
            
            <button
            onClick={() => navigate('/history')}
            className={`flex flex-col items-center gap-1 ${isActive('/history') ? 'text-secondary' : 'text-text-secondary-light dark:text-text-secondary-dark'}`}
            >
            <span className={`material-symbols-outlined ${isActive('/history') ? 'filled' : ''}`}>account_balance_wallet</span>
            <span className="text-[10px] font-bold">Revenue</span>
            </button>

            <button
            onClick={() => navigate('/profile')}
            className={`flex flex-col items-center gap-1 ${isActive('/profile') ? 'text-secondary' : 'text-text-secondary-light dark:text-text-secondary-dark'}`}
            >
            <span className={`material-symbols-outlined ${isActive('/profile') ? 'filled' : ''}`}>person</span>
            <span className="text-[10px] font-bold">Bursar</span>
            </button>
        </div>
        </div>
    );
